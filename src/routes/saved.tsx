...
function SavedDescriptions() {
  ...
  const categoryCounts = useMemo(() => {
    ...
    for (const l of listings) m.set(l.listingCategory || "Other", (m.get(l.listingCategory || "Other") ?? 0) + 1);
  ...
  const rows = useMemo(() => {
    ...
    const filtered = enriched
      ...
      .filter((r) => (category === "all" ? true : (r.listing.listingCategory || "Other") === category))
      .filter((r) =>
    ...
    <div className="min-h-screen w-full overflow-x-hidden bg-background pb-16">
      ...
      <main className="mx-auto max-w-6xl px-4 py-6 sm:px-6">
        <div className="grid grid-cols-2 gap-3 sm:grid-cols-3 lg:grid-cols-6">
          <StatCard index={1} label="Total Saved" value={stats.total} icon={<Layers className="h-4 w-4" />} />
          <StatCard
            index={2}
            label="Ready to List"
            value={stats.ready}
          ...
          />
          <StatCard index={5} label="Categories" value={stats.categories} icon={<Store className="h-4 w-4" />} />
          <StatCard index={6} label="Updated Today" value={stats.today} icon={<Clock className="h-4 w-4" />} />
        </div>
              ...
              >
                ...
                <div className="mt-auto pt-3">
                  <p className="text-[11px] text-muted-foreground">
                    Listed on {listed} account{listed === 1 ? "" : "s"} · {formatDate(l.updatedAt)}
                  </p>
                  <div className="mt-2 flex items-center gap-1.5">
                    ...
                    <button
                      type="button"
                      onClick={() =>
                        navigate({ to: "/list", search: { tab: "products" as const, sku: l.sku } })
                      }
