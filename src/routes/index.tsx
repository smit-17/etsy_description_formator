...
import { Check, ClipboardPaste, Copy, Eraser, Sparkles, Wand2 } from "lucide-react";
...
export const Route = createFileRoute("/")({
  head: () => ({
    meta: [
      { title: "Etsy Description Formatter — Unicode Bold Styling Tool" },
      {
        name: "description",
        content:
      ...
      { property: "og:title", content: "Etsy Description Formatter" },
      {
...
function Index() {
  ...
  const liveOutput = useMemo(() => formatEtsy(input, options), [input, options]);
  ...
  const actions = (
    <>
      <button
        type="button"
        onClick={() => {
          setManualOutput(formatEtsy(input, options));
          setLive(true);
        }}
    ...
    <div className="min-h-screen w-full overflow-x-hidden bg-background pb-28 md:pb-10">
        ...
        <div className="mx-auto max-w-6xl px-4 py-8 sm:px-6 sm:py-12">
          <div className="flex min-w-0 flex-wrap items-center justify-between gap-4">
            <div className="flex min-w-0 items-center gap-3">
              ...
              <div className="min-w-0">
                <h1 className="font-[family-name:var(--font-display)] text-2xl font-semibold tracking-tight text-navy-foreground sm:text-4xl">
                  Etsy Description Formatter
                </h1>
                <p className="mt-1 text-sm text-navy-foreground/70 sm:text-base">
                  Convert AI text into clean, styled, copy-ready Etsy descriptions.
                </p>
              </div>
            </div>
            <Link
              to="/prompts"
              className="inline-flex shrink-0 items-center gap-2 rounded-full border border-gold/60 px-4 py-2 text-sm font-medium text-gold transition-colors hover:bg-gold/10"
            >
              Saved AI Prompts
            </Link>
          </div>
      ...
      <main className="mx-auto max-w-6xl px-4 py-6 sm:px-6 sm:py-10">
        ...
        <p className="mt-6 text-center text-xs text-muted-foreground">
          Everything runs locally in your browser. Your wording, measurements and facts are never changed.
        </p>
      </main>
